...
export default function HotelOwnerDashboard() {
  ...
  const [bookingTrend, setBookingTrend] = useState<{ date: string; bookings: number }[]>([]);
        ...
        setRevenueTrend(
          past7Days.map(d => ({
            ...
            revenue: transactionsData.transactions
              .filter((t: Transaction) => new Date(t.createdAt).toDateString() === d.toDateString())
        ...
        setBookingTrend(
          past7Days.map(d => ({
            ...
            bookings: bookingsData.bookings
              .filter((b: Booking) => new Date(b.createdAt).toDateString() === d.toDateString())
              .length
          }))
        );
  ...
  const filterByDateRange = <T extends { createdAt: string }>(data: T[]) => {
  ...
  const exportCSV = <T extends object>(data: T[], fields: string[], filename: string) => {
    ...
    <div className="min-h-screen bg-[#0B0E14] text-white p-6 space-y-6">
      ...
      <div className="flex gap-4 items-end">
        ...
        <button
          onClick={() =>
            exportCSV(
              filterByDateRange(bookings),
              ["id", "customerName", "room", "amount", "createdAt"],
              "bookings"
            )
          }
            ...
            <tbody>
              {bookings.slice(-5).reverse().map(b => (
                <tr key={b.id}>
                  ...
                  <td className="px-3 py-1 border border-gray-700">
